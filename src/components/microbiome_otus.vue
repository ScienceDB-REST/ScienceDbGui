...
export default {
  ...
  methods: {
    ...
    downloadExampleCsv: function() {
      var t = this
      axios.get(t.$baseUrl() + '/microbiome_otus/example_csv', {
        headers: {
          'authorization': `Bearer ${t.$getAuthToken()}`,
          'Accept': 'application/json'
        },
        ...
        link.href = url;
        link.setAttribute('download', 'microbiome_otus.csv');
    ...
    downloadCsv: function() {
      var t = this
      axios.get(t.$baseUrl() + '/microbiome_otus/csv_export', {
        headers: {
          'authorization': `Bearer ${t.$getAuthToken()}`,
          'Accept': 'application/json'
        },
        ...
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'exported_microbiome_otus.csv');
        document.body.appendChild(link);
        link.click();
      }).catch(res => {
        var err = (res && res.response && res.response.data && res.response

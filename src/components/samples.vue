...
export default {
  ...
  methods: {
    ...
    onDelete () {
      if (window.confirm("Do you really want to delete samples of ids '" + this.$refs.vuetable.selectedTo.join("; ") + "'?")) {
        var t = this;
        var url = this.$baseUrl()() + '/sample/' + this.$refs.vuetable.selectedTo.join("/")
        axios.delete(url, {
          headers: {
